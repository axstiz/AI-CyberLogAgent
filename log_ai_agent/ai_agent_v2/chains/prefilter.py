...
def update_failed_attempts_tracking(log_entry: ParsedLog, tracking_dict: dict):
    ...
    failed_indicators = [
        ...
        r'password.*invalid', r'authentication.*failed', r'failed.*auth',
        r'access.*denied', r'denied.*access'
    ]
    
    is_failed_attempt = any(re.search(pattern, combined) for pattern in failed_indicators)
    ...
    if is_failed_attempt:
        ...
        success_indicators = [
            r'successful.*login', r'login.*successful', r'accepted.*password',
            r'password.*accepted', r'session.*opened', r'login.*session'
        ]
