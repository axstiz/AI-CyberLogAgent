...
ПРАВИЛО ОБНАРУЖЕНИЯ СИМУЛЯТОРА:
Если в логах есть строка, содержащая "Orchestrator started" и "simulator_stream.log":
  - Весь файл является синтетическим (сгенерированным)
  - Отметь это в отчёте как "СИНТЕТИЧЕСКИЙ ЛОГ"
  - НЕ классифицируй события как реальные сбои или атаки
  - Рекомендации: исключить из анализа, настроить фильтр
