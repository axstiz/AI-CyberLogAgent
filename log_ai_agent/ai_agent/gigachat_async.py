...
async def get_last_messages(conn: asyncpg.Connection, user_id: int, limit: int = 20) -> list[dict]:
...
async def process_chat_message(user_id: int, user_message: str, database_url: str) -> str:
    """
    ...
    try:
        ...
        logger.info(f"GigaChat context: загружено {len(history)} сообщений для пользователя {user_id}")
...
async def clear_user_context(user_id: int, database_url: str) -> int:
    ...
    try:
        ...
        logger.info(f"GigaChat context: очищено {deleted_count} сообщений для пользователя {user_id}")
