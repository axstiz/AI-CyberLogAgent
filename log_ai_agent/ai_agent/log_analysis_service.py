...
async def save_log_to_db(conn: asyncpg.Connection, file_content: str, filename: str) -> int:
    """
    Сохранить лог в БД.
...
async def create_report_from_analysis(
    ...
    if "---META---" in gigachat_response:
        ...
        logger.warning("Метаданные не найдены в ответе GigaChat, используем значения по умолчанию")
...
async def process_log_file(
    ...
    try:
        ...
        logger.info(f"Классический анализ завершен: найдено {len(incidents)} инцидентов")
        ...
        logger.info(f"Загружено {len(threat_types)} типов угроз и {len(severity_levels)} уровней серьезности")
